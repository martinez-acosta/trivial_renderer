#include "tvector3d.h"
constexpr TVector3D::TVector3D() : x(0.0f), y(0.0f), z(0.0f) {}

constexpr TVector3D::TVector3D(float x, float y, float z) : x(x), y(y), z(z) {}

constexpr TVector3D::TVector3D(const TPoint &p) : x(p.x), y(p.y), z(0.0f) {}
...
constexpr TPoint TVector3D::toPoint() const {
...
constexpr TVector3D TVector3D::crossProduct(const TVector3D &v1,
                                            const TVector3D &v2) const {
  return TVector3D(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
                   v1.x * v2.y - v1.y * v2.x);
}

constexpr TVector3D TVector3D::normalize() const {
